...
async function upsertChallengeResponse(full_nonce: string, user_id: string, challengeResponse: string, currentTime: bigint) {
    ...
    `;
    log(globalVar.getReferenceId(), "Executing upsert query for challenge response:", upsertQuery);
    try {
        await pool.query(upsertQuery, [full_nonce, user_id, challengeResponse, currentTime]);
        log(globalVar.getReferenceId(), "Challenge response upserted successfully.");
        return "success";
    } catch (upsertError) {
        log(globalVar.getReferenceId(), "Error during upsert challenge response:", upsertError);
...
export async function handleLoginRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
        ...
        if (missingFields.length > 0) {
            ...
            log(globalVar.getReferenceId(), "Missing fields:", JSON.stringify(missingFields));
            return;
        }
        if (half_nonce.length !== 8) {
            ...
            log(globalVar.getReferenceId(), "half_nonce must be 8 characters long");
            return;
        }
        ...
        if (userResult.rowCount === 0) {
            ...
            const fakeSalt = generateRandomString(26);
            const fakeIterations = 0;
            log(globalVar.getReferenceId(), `Username incorrect: generating fake data. [Fake full_nonce:, ${fakeFullNonce},  Fake salt: ${fakeSalt}]`);
            res.json({
                full_nonce: fakeFullNonce,
                salt: fakeSalt,
                iterations: fakeIterations
            });
            log(globalVar.getReferenceId(), "User not found for username:", username);
            return;
        }
        ...
        if ((challengeResult.rowCount ?? 0) > 0) {
            ...
            if ((currentTime - BigInt(existingChallenge.tstamp)) < 10) {
                ...
                log(globalVar.getReferenceId(), "res.status(429).json:", {
                    error_code: "42900001",
        ...
        log(globalVar.getReferenceId(), "Calculated challenge response.");

        log(globalVar.getReferenceId(), "Generated full_nonce:", full_nonce);
        log(globalVar.getReferenceId(), "User id:", user.id);
        log(globalVar.getReferenceId(), "Challenge response:", challengeResponse);
        log(globalVar.getReferenceId(), "Current Time:", currentTime);

        try {
            const upsertStatus = await upsertChallengeResponse(full_nonce, user.id, challengeResponse, currentTime);
            log(globalVar.getReferenceId(), "Upsert status:", upsertStatus);
        } catch (error) {
            log(globalVar.getReferenceId(), "Error during upsert challenge_response:", error);
        ...
        log(globalVar.getReferenceId(), " res.status(200) : Response sent:", {
            full_nonce,
            salt: user.salt,
            iterations: user.iterations
        });
        ...
        log(globalVar.getReferenceId(), "Error during handling login request:", e);

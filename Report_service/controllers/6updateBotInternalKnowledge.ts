...
export const updateBotInternalKnowledge = async (req: Request, res: Response) => {
    let referenceId = globalVar.getReferenceId() || 'undefined';
    log(referenceId, "Executing method: updateBotInternalKnowledge");

    const realBackendURL = process.env.endpoint6 ?? '';
    ...
    if (validationResult === "0") {
        ...
        log(referenceId, `Response sent`, `: res.status(401).json({
            error_message: "unauthenticated",
            error_code: "40000052"
